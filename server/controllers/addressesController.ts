...
// Get user address - GET/api/addresses
...
export const addAddresses = async (req: Request, res: Response) => {
    ...
    if (lat == null || lng == null ){
        return res.status(400).json({message: "Location coords are required. Please allow location access"})
    }
    ...
    await prisma.address.create({
        data: {
            userId: req.user!.id,
            label, address, city, state, zip, isDefault,
            lat: Number(lat),
            lng: Number(lng)
        }
    })
