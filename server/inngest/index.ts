...
export const inngest = new Inngest({ id: 'supermercato' });
    ...
    const { deals, users } = await step.run(
      ...
      async () => {
        ...
        const products = await prisma.product.findMany({
          select: { name: true, price: true, originalPrice: true, image: true },
        });
...
const autoAssignRider = inngest.createFunction(
  ...
  async ({ event, step }) => {
    ...
    // wait 5 mins before attempting assignment
    await step.sleep('wait-5-min', '5m');

    const assignment = await step.run('assign-rider', async () => {
      const order = await prisma.order.findUnique({ where: { id: orderId } });

      // Skip if order does not exist, already assigned, or cancelled
