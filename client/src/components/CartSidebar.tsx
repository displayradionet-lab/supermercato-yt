...
const CartSidebar = () => {
  ...
  return (
    ...
    shadow-2xl flex flex-col animate-slide-iin-right"
      >
        {/* Header */}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-4">
          {items.length === 0 ? (
            <div className="flex flex-col items-center justify-center h-full text-center">
              <ShoppingCartIcon className="text-app-border mb-4 size-16" />
        ...
        {items.length > 0 && (
          <div className="p-5 border-t border-app-border space-y-3">
            <div className="flex justify-between text-sm">
              <span className="text-app-text-light">Sub-total</span>
              <span className="font-medium">
                {currency} {cartTotal.toFixed(2)}
              </span>
            </div>
            <div className="flex justify-between text-sm">
              <span className="text-app-text-light">Spedizione</span>
              <span className="font-medium">
                {deliveryFee === 0 ? (
                ...
                {deliveryFee > 0 && <p className='text-sm text-app-text-light text-center'>
                    Spedizione gratuita per ordini oltre i 20 dollari
                    </p>}

                    <div className="flex justify-between text-base font-semibold border-t 
                border-app-border pt-3">
                    <span>Totale</span>
                ...
                >
                    Procedi al pagamento <ArrowRightIcon className='size-4'/>
                </button>
          </div>
        )}
      </div>
