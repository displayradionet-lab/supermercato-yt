...
export default function AdminOrders() {
  ...
  useEffect(() => {
    ...
    // 2. Polling in background ogni 5 secondi (senza attivare il loading)
    const interval = setInterval(() => {
      fetchOrders();
      fetchPartners();
    }, 5000);

    // 3. Pulizia dell'intervallo quando si cambia pagina
  ...
  const handleStatusChange = async (id: string, newStatus: string) => {
    try {
      ...
      await fetchOrders(); // Ricarica la lista aggoirnata
    } catch (error: any) {
  ...
  const handleAssign = async () => {
    if (!assignModal || !selectedPartner || actionLoading) return;
    try {
      setActionLoading(true);
                  ...
                  >
                    ...
                    <td className="px-6 py-4">
                      {order.deliveryPartner ? (
                        ...
                        <button
                          ...
                          className="px-3 py-1.5 text-xs font-medium bg-indigo-50 text-indigo-700 rounded-lg
                    ...
                    <td className="px-6 py-4">
                      <select
                        ...
                        className={`px-3 py-1.5 rounded-lg text-xs font-semibold border-r-8 border-transparent 
                                                    outline-none cursor-pointer leading-tight ${statusColors[order.status] || 'bg-zinc-100 text-zinc-800'}`}
                      >
                        {statusOptions.map((s) => (
